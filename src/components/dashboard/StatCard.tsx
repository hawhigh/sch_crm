...
export const StatCard = ({
  ...
  return (
    <div className={cn("rounded-none p-10 flex flex-col justify-between min-h-[240px] relative overflow-hidden transition-all duration-500 hover:scale-[1.02] cursor-default border border-transparent shadow-xl", themes[variant], className)}>
      {/* Decorative Glow */}
      <div className={cn("absolute -top-12 -right-12 w-48 h-48 blur-[60px] rounded-none mix-blend-overlay opacity-20 pointer-events-none", isDarkBase ? "bg-white" : "bg-black")} />
      
      <div className="flex justify-between items-start relative z-10 gap-4">
        <div className={cn("w-16 h-16 rounded-none shrink-0 flex items-center justify-center shadow-lg", isDarkBase ? "bg-white/10" : "bg-surface-low group-hover:bg-primary group-hover:text-white transition-colors")}>
        ...
        <div className="text-right min-w-0">
          ...
          <div className="flex items-baseline justify-end gap-3 flex-wrap">
             ...
             {trend && <span className={cn("text-[9px] font-black mb-1 uppercase tracking-[0.1em] px-2 py-0.5 rounded-none", isDarkBase ? "bg-white/10 text-[var(--secondary)]" : "bg-primary/5 text-primary")}>{trend}</span>}
          </div>
        </div>
      </div>
